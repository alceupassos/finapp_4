/**
 * Script de Importação F360 2025 - Inserção Direta via MCP Supabase
 * 
 * Este script importa dados do F360 e insere diretamente via MCP Supabase
 * em batches para evitar problemas de tamanho.
 */
...
async function importCompanyMonth(cnpj, companyId, month) {
  ...
  for (const entry of entries) {
    ...
    const dateStr = competenciaDate.split('T')[0]
    ...
    if (entry.Liquidacao) {
      const liquidacaoStr = entry.Liquidacao.split('T')[0]
      dfcEntries.push({
        company_id: companyId,
        company_cnpj: entryCnpj,
        date: liquidacaoStr,
    ...
    const entryDateStr = (entry.DataDoLcto || competenciaDate).split('T')[0]
...
async function main() {
  console.log('🚀 Importação F360 2025 - Geração de SQL para MCP\n')
  ...
  fs.writeFileSync('f360_import_october.sql', fullSQL)
  console.log('✅ SQL salvo em f360_import_october.sql')
  console.log(`\n📊 Resumo:`)
  ...
  console.log(`\n💡 Execute o SQL via MCP Supabase usando mcp_supabase_apply_migration`)
}
