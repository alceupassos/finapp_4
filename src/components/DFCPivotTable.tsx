...
export function DFCPivotTable({ cnpj, period = 'Ano' }: DFCPivotTableProps) {
  ...
  return (
    <div className="card-premium p-5">
      <h3 className="text-sm font-semibold mb-4">Fluxo de Caixa (DFC)</h3>
              ...
              pivot.map((row) => (
                ...
                >
                  ...
                  {row.months.flatMap((month, idx) => [
                    <td key={`${idx}-entrada`} className="p-3 text-right text-emerald-400">
                      {formatCurrency(month.entrada)}
                    </td>,
                    <td key={`${idx}-saida`} className="p-3 text-right text-red-400">
                      {formatCurrency(month.saida)}
                    </td>,
                    <td key={`${idx}-saldo`} className={`p-3 text-right ${
            ...
            expandedCategories.has(row.category) && row.children ? (
              ...
              >
                {row.children.map((child, idx) => (
                  ...
                  >
                    <td className="p-3 pl-8 text-muted-foreground text-xs">{child.descricao}</td>
                    {child.months.flatMap((month, monthIdx) => [
                      <td key={`${monthIdx}-entrada`} className="p-3 text-right text-xs text-emerald-400">
                        {formatCurrency(month.entrada)}
                      </td>,
                      <td key={`${monthIdx}-saida`} className="p-3 text-right text-xs text-red-400">
                        {formatCurrency(month.saida)}
                      </td>,
                      <td key={`${monthIdx}-saldo`} className={`p-3 text-right text-xs ${
                        month.saldo < 0 ? 'text-red-400' : 'text-emerald-400'
                      }`}>
                        {formatCurrency(month.saldo)}
                      </td>,
